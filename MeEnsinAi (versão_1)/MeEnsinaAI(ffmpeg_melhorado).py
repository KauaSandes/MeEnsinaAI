...
# API settings
OPENROUTER_API_KEY = "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"
...
def analyze_gameplay_with_llama(frames, sample_size=5):
    ...
    for i, frame in enumerate(selected_frames):
        try:
            ...
            # Inside the analyze_gameplay_with_llama function
            prompt = (
                "Você é um especialista em Minecraft Dungeons. Analise este frame de gameplay e forneça feedback técnico e específico. "
                "Concentre-se em: "
                "- Posicionamento do jogador (está bem posicionado ou exposto?). "
                "- Uso de artefatos e habilidades (estão sendo usados no momento certo?). "
                "- Priorização de inimigos (quais inimigos deveriam ser enfrentados primeiro?). "
                "- Gestão de recursos (vida, flechas, poções). "
                "- Erros comuns do jogador e como corrigi-los. "
                "Forneça dicas práticas e objetivas, com exemplos concretos baseados no frame."
            )
...
def synthesize_tips_with_gemini(raw_tips):
    ...
    try:
        ...
        prompt = (
            "Você é um assistente de jogos especializado em Minecraft Dungeons. Recebi as seguintes análises de gameplay de um modelo de visão: \n\n"
            f"{'\n'.join(raw_tips)}\n\n"
...
print("Iniciando análise de gameplay com Llama...")
raw_tips = analyze_gameplay_with_llama(frames_brutos, sample_size=5)
