...
def record_game_window(game_exe, duration=60):
    ...
    try:
        ...
        if not game_process:
            print(f"Processo do jogo {game_exe} não encontrado. Certifique-se de que o jogo está em execução.")
            return False
        ...
        with mss.mss() as sct:
            ...
            monitor = sct.monitors[1]  # monitor[1] é geralmente o monitor primário
...
def run_gameplay_analysis(game_exe, game_name):
    ...
    try:
        ...
        for i, (frame, timestamp) in enumerate(zip(frames, timestamps), 1):
            analysis = analyze_frame(frame, i, total_frames, timestamp, game_prompt)
        ...
        return {"success": True, "report": synthesis}
